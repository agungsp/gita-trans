<?php
    ...
    $no_invoiceFULL = invoiceFormat($no_invoice).'/'.date("m/y");
?>
    ...
    <body id="bg" style="height:100%;">
        ...
        <nav class="nav navbar-default" style="background-color:#337ab7;">
            <div class="container-fluid">
                ...
                <div class="navbar-header col-xs-4">
                    <h4 style="text-align:center;font-weight:bold;color:#fff;">Invoice<br> <?php echo $no_invoiceFULL; ?></h4>
                </div>
            </div>
        </nav>
        ...
        <div class="container-fluid wrapper">
            <div class="col-xs-3 header">
                ...
                <form class="" action="proses.php" method="post" style="margin-top:30px;">
                    <div class="">
                        <?php
                            echo '<input type="hidden" name="invoice" value="'.$no_invoiceFULL.'" readonly="readonly">';
                        ?>
                    </div>
            ...
            <div class="col-xs-9 header">
                <div class="" style="overflow:auto; padding: 5px; height: 270px;">
                    <table class="table table-responsive">
                        ...
                        <tbody>
                            <?php
                                $query = mysql_query("SELECT deskripsi,jumlah,satuan,harga_satuan,harga_total FROM transaksi where no_invoice = '$no_invoiceFULL'") or die(mysql_error());
                                if (mysql_num_rows($query) == 0) {
                                    ...
                                    while ($data = mysql_fetch_assoc($query)) {
                                        echo '<tr>';
                                            ...
                                            echo '<td class="mainText"><a class="btn btn-danger" href="hapus.php?noInvoice='.$no_invoiceFULL.'&desk='.$data['deskripsi'].'"><i class="fa fa-trash-o"></i></td>';
                                        echo '</tr>';
                                        $no++;
                                    }
                                }
                ...
                <div class="input-group col-xs-4 col-xs-offset-8" style="margin-top:6px;">
                    <?php
                        $query_transaksi = mysql_query("SELECT SUM(harga_total) AS total FROM transaksi where no_invoice = '$no_invoiceFULL'");
                ...
                <div class="form-group" style="margin-top:7px;float:right;">
                    <?php
                        echo '<a href="simpan.php?Invoice='.$no_invoiceFULL.'&pengirim='.$_SESSION['pengirim'].'&alm_pengirim='.$_SESSION['alm_pengirim'].'&penerima='.$_SESSION['penerima'].'&alm_penerima='.$_SESSION['alm_penerima'].'&kapal='.$_SESSION['kapal'].'&total='.$total_transaksi['total'].'" class="btn btn-primary"><b><i class="fa fa-save"></i><br>Simpan</b></a>';
                        echo '<a style="margin-left:5px;" href="simpan.php?Invoice='.$no_invoiceFULL.'&pengirim='.$_SESSION['pengirim'].'&alm_pengirim='.$_SESSION['alm_pengirim'].'&penerima='.$_SESSION['penerima'].'&alm_penerima='.$_SESSION['alm_penerima'].'&kapal='.$_SESSION['kapal'].'&total='.$total_transaksi['total'].'" onclick="print_popup()" class="btn btn-success"><b><i class="fa fa-print"></i><br>Simpan & Cetak</b></a>';
                    ?>
                </div>
            </div>
        </div>
